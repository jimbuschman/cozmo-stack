...
/// <summary>
/// Restores a decodable JPEG from the robot's "minimized" JPEG stream.
///
/// The camera strips the whole JFIF header and the 0x00 stuffing byte that JPEG requires after each 0xFF
/// inside entropy-coded data. The headers below are the ones the encoder assumed: standard JFIF, the
/// quantisation table the robot uses, and the standard luminance Huffman tables, with the frame size
...
public static class MiniJpeg
{
    ...
    public const byte EncodingJpegColor = 6;
    public const byte EncodingJpegMinimizedGray = 8;
    public const byte EncodingJpegMinimizedColor = 9;

    private static readonly byte[] GrayHeader =
    {
    ...
    private static readonly byte[] ColorHeader =
    {
        0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x00, 0x00, 0x01,
        0x00, 0x01, 0x00, 0x00, 0xFF, 0xDB, 0x00, 0x43, 0x00, 0x10, 0x0B, 0x0C, 0x0E, 0x0C, 0x0A, 0x10,
        0x0E, 0x0D, 0x0E, 0x12, 0x11, 0x10, 0x13, 0x18, 0x28, 0x1A, 0x18, 0x16, 0x16, 0x18, 0x31, 0x23,
        0x25, 0x1D, 0x28, 0x3A, 0x33, 0x3D, 0x3C, 0x39, 0x33, 0x38, 0x37, 0x40, 0x48, 0x5C, 0x4E, 0x40,
        0x44, 0x57, 0x45, 0x37, 0x38, 0x50, 0x6D, 0x51, 0x57, 0x5F, 0x62, 0x67, 0x68, 0x67, 0x3E, 0x4D,
        0x71, 0x79, 0x70, 0x64, 0x78, 0x5C, 0x65, 0x67, 0x63, 0xFF, 0xC0, 0x00, 0x11, 0x08, 0x00, 0xF0,
        0x01, 0x40, 0x03, 0x01, 0x21, 0x00, 0x02, 0x11, 0x00, 0x03, 0x11, 0x00, 0xFF, 0xC4, 0x00, 0xD2,
    ...
    public static byte[] ToJpeg(ReadOnlySpan<byte> payload, int width, int height, byte encoding)
    {
        ...
        BinaryPrimitives.WriteUInt16BigEndian(header.AsSpan(off), (ushort)height);
        BinaryPrimitives.WriteUInt16BigEndian(header.AsSpan(off + 2), (ushort)width);
        ...
        // The first payload byte is not part of the entropy stream.
        for (int i = 1; i < end; i++)
        {
            byte b = payload[i];
            outBuf.Add(b);
            if (b == 0xFF) outBuf.Add(0x00);   // reinstate JPEG byte stuffing
        }
        outBuf.Add(0xFF);
        outBuf.Add(0xD9);                      // end of image
