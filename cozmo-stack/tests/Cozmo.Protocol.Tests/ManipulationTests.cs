    ...
    private sealed class Rig : IDisposable
    {
        ...
        public Rig()
        {
            ...
            M.TurnOverride = (id, max, ct) =>
            {
                ...
                if (Vision.World.GetLocatedObjectById(id) is { } o) { var d = o.Pose.Translation - new Vec3(X, Y, 0); Angle = (float)Math.Atan2(d.Y, d.X); State(); }
        ...
        {
            T += 33;
            Send(new RobotState { Timestamp = T, Pose = new RobotPose { X = X, Y = Y, Angle = Angle }, HeadAngle = Head, Status = flags ?? 0,
        ...
        public VisionFrameResult Frame()
        {
            ...
            frame.Fill(150);
            if (Cube is { } c && Lib is not null) MarkerRenderer.DrawCube(frame, Lib, new CameraModel(Cal, pd.CameraPose), ObjectType.Block_LIGHTCUBE1, c);
            ...
            foreach (var m in fresh)
            {
                switch (m)
                {
                    case ExecutePath ep:
                        ...
                        var turn = Sent.OfType<AppendPathSegmentPointTurn>().LastOrDefault();
                        if (turn is not null && Sent.IndexOf(turn) > Sent.IndexOf(Sent.OfType<ClearPath>().Last())) Angle = BitConverter.UInt32BitsToSingle(turn.Field2);
                        State();
                    ...
                    case DockWithObject:
                        ...
                        _dockPending = true; _signals = 0;
                        break;
                    case DockingErrorSignal when _dockPending:
        ...
        private bool _dockPending; private int _signals;
