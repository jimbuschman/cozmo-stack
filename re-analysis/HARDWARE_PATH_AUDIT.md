...
| MOV | `drive --allow-drive` | `Control.Drive` → normal `CozmoMotion` head/lift/wheel/stop APIs | motor action acknowledgements, wheel `RobotState`, confirmed stop | VALID |
...
| E | `camera --color` | `Devices.Camera` → `CozmoRobot.StartCamera` → chunk reassembly/mini-color JPEG → `CameraFrame.Save` | `EnableColorImages`, `ImageRequest`, inbound `ImageChunk`; raw 160×240 JPEG retained and saved 320×240 presentation geometry | VALID |
...
| P | `manip --roll` | `DockHelper` → `RollObjectAction` | dock/result exchange, terminal action success and observed cube up-axis change | VALID |
...
| U | `manip --wheelie` | shipped `PopAWheelieBehavior` | dock/wheelie action terminal success and cliff-stop restoration | VALID |
...
- Camera encoded geometry: `EncodedImage::AddChunk` 0x004F1CE0 and `MiniColorToJpeg` table
  ...
  presentation representations are now kept distinct. The interpolation detail is an equivalent app-side
  presentation implementation; M3-016 remains `HARDWARE_ONLY` until the color result is observed.
