# Handoff — 2026-09-20 (source-fidelity cleanup: Pass 1, then M9)

## Where things stand

> **Read the manifest before the tables below.** [`fidelity_manifest.json`](fidelity_manifest.json) is
> now the authoritative statement of what rests on primary source: 195 records, of which **69 are
> RECOVERABLE_GAP on a live execution path**. The prose in this file and in the milestone documents was
> written before it and says "complete offline" in places the manifest does not support. Where they
...
| Tests | **710 tests** (`dotnet test Cozmo.sln`, about 4 m; 651 before the fidelity pass), all passing offline. The runner is capped at two parallel threads — several behaviour rigs drive against a wall-clock budget and the default parallelism starved them on a four-core machine |
...
| Next | **The remaining 69 live-path RECOVERABLE_GAPs**, subsystem by subsystem, the way M9 was done; `FIDELITY_GAPS.md` lists every one with what would settle it. Then **the hardware / integration validation phase** (items A–Z, Y blocked) through `hardware-test`, then the architectural work listed under "Next task" |
