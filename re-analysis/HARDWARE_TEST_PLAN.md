...
| R | stack two cubes | M12 | `dotnet run --project src/Cozmo.Conformance -- manip 172.31.1.1 --stack --acceptance --obb <obb>` with two connected upright cubes in view (`--obb` is now required: the tool loads the animation library from it and waits for **two** located cubes, reporting what is missing rather than starting) | phases `PickingUpBlock` → `StackingBlock` → `PlayingFinalAnim` | he picks one up, carries it to the other and places it on top |
...
| T | knock over a stack | M13 | `manip 172.31.1.1 --knockover --obb <obb> --acceptance` with two connected cubes stacked (the tool now refuses to start until the world model actually holds a stack, and says so) | `stacks: 7/8`, `reach for block`, `KnockOverGrabAttempt`, `DriveAndFlipBlockAction`, `the stack came apart`, `KnockOverSuccess` | he turns to the stack, drives to 85 mm, reaches, flips the bottom cube and the stack falls; the success animation plays |
| U | pop a wheelie | M13 | `manip 172.31.1.1 --wheelie --obb <obb> --acceptance` (an upright cube ahead; `--obb` is required for the animation library) | `PopAWheelieInitial`, `Docking with marker … using action PopAWheelie`, `PoppedWheelie`, `EnableStopOnCliff(true)` on stop | he docks, rides up onto the cube's edge and drops back; a miss plays the realign/retry animation and tries again (up to 3) |
