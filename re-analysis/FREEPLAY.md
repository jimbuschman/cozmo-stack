...
* `PickNewActivity` tries the desired-from-objects activity before the priority order (INFERRED); the engine's
  ...
  instruction.
* `needsActionID` is registered when the activity ends (INFERRED hook); sparks, stars and the sparks economy
  are the app's (`SparksRewardPending` is a flag).
* `boredomMultiplier`, the feature gate, the pyramid strategy's 0.4 random factor, decay modifiers, damaged
  parts and persistence: DEFERRED.
