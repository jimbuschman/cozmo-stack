...
L = ["# Robot protocol status",
     ...
     "|---|---|---|",
     "| hardware verified | %d | exercised on the firmware-2457 robot: the robot sent it and our codec re-encoded it byte-identically, or the robot demonstrably acted on it |" % ver["hardware_verified"],
     "| capture verified | %d | seen on the wire in a real session with a consistent length, but no response ties it to robot behaviour |" % ver["capture_verified"],
