...
- **Next (in order; the manager takes the first open item):**
  ...
  4. [ ] **Continue M6** from `re-analysis/M6-STATUS.md` and `re-analysis/workplans/M6-plan.md` (§8: M6-017 audio-thread frame model and M6-008 continuous containers, then finish M6-009's value store, then batches 3–5). Do not wire the runtime into `WwisePlayback`/`WwiseAudioSource`/`WwiseSongRenderer`/`AnimationScheduler` until the live voice/mixer (batches 3–4) exists.
