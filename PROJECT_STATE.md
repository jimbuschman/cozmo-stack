...
- **Batch 1 done (7aba301). Batch 2a done** (e6f2ed7; receive path: B17 truncation, receive-error counters, partial-header overrun after the header, M1-038); records it repaired are settled in one verified pass at the end of batch 2. **Batch 2b-i done** (clock, construction-time 2 ms scheduler + FIFO executor, posted sends, RobotLink isolation; three verification passes).
...
- **M1-019, B21:** what time the Disconnect closure passes to SendMessage (the code passes NaN; the time is discarded). Also: the sync-mode QueueMessage time 0.0 and ChangeSyncMode(true) keeping the queue come from a verifier reading, not a frozen row (0x00836B48, 0x00836862..0x00836878).
